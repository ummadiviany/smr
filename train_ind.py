...
if wandb_log:
    wandb.login()
    wandb.init(project="Spleen_Individual", entity="vinayu", config = config)
...
def validate(test_loader : DataLoader, dataset_name : str = None):
    ...
    with torch.no_grad():
        ...
        if wandb_log and log_images and epoch % img_log_interval == 0 and dataset_name == curr_dname:
